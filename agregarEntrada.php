...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST) and (!empty($_POST))) {
        ...
        try {  //Comprobamos que el nombre de la categoría no exista ya               
            $sql3 = "SELECT nombre FROM categorias";
            $nombreEncontrado = false; //Variable para definir si encuentra nombre

            // Preparamos la consulta...
            $query = $conexion->query($sql3);
            //Supervisamos si la consulta se realizó correctamente... 
            if ($query) {
                $msgresultado = '<div class="alert alert-success">' . "El listado de nombre de categorias realizó correctamente!! :)" . '</div>';
                //Si el nombre de la categoría coincide con alguno de laq tabla
                while ($fila1 = $query->fetch()) {
                    if ($nombre_categoria == $fila1['nombre']) {
                        $nombreEncontrado = true;
                        $msgresultadoNombre = '<div class="alert alert-success">' . "El nombre de la categoría ya existe" . '</div>';
                    }
                }
            }
        ...
        //Si el nombre encontrado de la categoría no existe---------------------
        if (!$nombreEncontrado) {
                    ...
                    try {  //Definimos la instrucción SQL parametrizada 
                        ...
                        if ($query) {
                            $msgresultado = '<div class="alert alert-success">' .
                                "Los datos de categorías se obtuvieron correctamente!! :)" . '</div>';
                        } // o no :( 
                    } catch (PDOException $ex) {
                        $msgresultado = '<div class="alert alert-success">' .
                            "No se pudieron obtener los datos de categoría!! :( <br/>(" .
                            $ex->getMessage() . ')</div>';
                        die();
                    }
        ...
        }else { //Si existe el nombre de la categoría-------------------------------------------------------
            if (count($errors) == 0) {
                //RECUPERAR ID DE CATEGORIAS POR NOMBRE
                try {  //Definimos la instrucción SQL parametrizada 
                    $sql2 = "SELECT id FROM categorias where nombre=:nombre_categoria";
        ...
        <div class="container">
            ...
            <form action="agregarEntrada.php" method="POST" enctype="multipart/form-data">
                ...
                <label for="titulo">Título
                    <input class="my-4 mx-2" type="text" name="titulo" class="form-control" <?php if (isset($_POST["titulo"])) {
                ...
                <label for="nombre_categoria">Nombre Categoría
                    <input class="my-4 mx-2" type="text" name="nombre_categoria" class="form-control" <?php if (isset($_POST["nombre_categoria"])) {
                                                                                                            echo "value='{$_POST["nombre_categoria"]}'";
                                                                                                        } ?> />
                    <?php echo  mostrar_error($errors, "nombre_categoria"); ?>
                    <?php echo $msgresultadoNombre; ?>
                </label>
                <br />

                <!--Campo Imagen-->
                <label for="image">Imagen:
                    <input class="my-2 mx-2" type="file" name="image" id="image" class="form-control-file" />
